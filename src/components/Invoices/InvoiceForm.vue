      ...
      <div v-if="!isEditing" class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
        <div>
          ...
          <select
            ...
            @change="handleShippingCompanyChange"
            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            required
          >
            ...
            <option value="Manual">Manual Entry</option> </select>
        </div>
        <div>
            ...
            <input
                ...
                step="0.01"
                min="0"
                :readonly="localInvoice.shipping_company !== 'Manual'"
                :class="{'bg-gray-100 cursor-not-allowed': localInvoice.shipping_company !== 'Manual'}"
            />
        </div>
      </div>
...
const localInvoice = reactive({
  ...
  total: 0,
  status: 'draft',
  shipping_company: 'FedEx', // Default for new invoices
  shipping_cost: 0, // Default shipping cost
});

const getShippingCostForCompany = (company) => {
  switch (company) {
    case 'DHL': return 12.5;
    case 'UPS': return 15;
    case 'FedEx': return 10;
    default: return 0; // For 'Manual' or other unrecognized
  }
};

const handleShippingCompanyChange = () => {
  if (localInvoice.shipping_company !== 'Manual') {
    localInvoice.shipping_cost = getShippingCostForCompany(localInvoice.shipping_company);
  } else {
    localInvoice.shipping_cost = 0; // Reset for manual entry
  }
  calculateTotals();
};
...
const updateItemPrice = (index) => {
  ...
  if (selectedProduct) {
    ...
    localInvoice.items[index].unit_price = 0.00;
  }
  calculateTotals();
};
...
const calculateTotals = () => {
  ...
  // Calculate total: subtotal + shipping cost (only for new invoices, for edited, it's part of fetched data)
  localInvoice.total = sub + (isEditing.value && fetchedInvoiceFromComposable.value?.shipping?.shipping_cost !== undefined
    ? fetchedInvoiceFromComposable.value.shipping.shipping_cost
    : (Number(localInvoice.shipping_cost) || 0));
};
  ...
  if (isEditing.value && route.params.id) {
    ...
    if (fetchedInvoiceFromComposable.value) {
      Object.assign(localInvoice, {
        id: fetchedInvoiceFromComposable.value.id,
        reference_number: fetchedInvoiceFromComposable.value.reference_number,
        customer_id: fetchedInvoiceFromComposable.value.customer_id,
        date: fetchedInvoiceFromComposable.value.date,
        status: fetchedInvoiceFromComposable.value.status,
        items: fetchedInvoiceFromComposable.value.items.map(item => ({
          product_id: item.product.id,
          quantity: item.quantity,
          unit_price: Number(item.unit_price).toFixed(2),
        })),
        // When editing, shipping_company and shipping_cost are read from the fetched invoice's shipping relation.
        // We don't pre-fill localInvoice.shipping_company/cost directly for editing forms,
        // as they are primarily for creation. The total calculation handles it.
      });
      // Ensure total is correctly calculated based on fetched shipping cost for existing invoices
      localInvoice.total = localInvoice.subtotal + (fetchedInvoiceFromComposable.value.shipping?.shipping_cost || 0);
    ...
    Object.assign(localInvoice, {
      ...
      total: 0,
      status: 'draft',
      shipping_company: 'FedEx', // Default shipping company for new invoices
      shipping_cost: getShippingCostForCompany('FedEx'), // Set default shipping cost initially
    });
    addItem();
  }
  calculateTotals();
...
// Watch localInvoice.items and shipping_cost for changes to recalculate totals
watch([() => localInvoice.items, () => localInvoice.shipping_cost, () => localInvoice.shipping_company], calculateTotals, { deep: true });
...
const submitForm = async () => {
  ...
  if (localInvoice.items.some(item => Number(item.unit_price) <= 0)) {
  ...
  try {
    if (isEditing.value) {
      ...
      const createPayload = {
        ...
        shipping_company: localInvoice.shipping_company === 'Manual' ? 'Custom' : localInvoice.shipping_company, // Send 'Custom' if manually entered
        shipping_cost: localInvoice.shipping_cost,
      };
...
const localInvoice = reactive({
  ...
  status: 'draft',
  shipping_company: '',
  // shipping_cost: 0, // Removed this from reactive state as it's derived
});

// Function to calculate shipping cost based on selected company (for new invoices)
const calculateShippingCost = () => {
  switch (localInvoice.shipping_company) {
    case 'DHL':
      return 12.5;
    case 'UPS':
      return 15;
    case 'FedEx': // Default or explicitly set
    default:
      return 10;
  }
};
...
const calculateTotals = () => {
  ...
  localInvoice.subtotal = sub;
  localInvoice.total = sub + (isEditing.value ? 0 : calculateShippingCost()); // Add shipping cost only for new invoices
};
...
watch([() => localInvoice.items, () => localInvoice.shipping_company], calculateTotals, { deep: true });
  ...
  try {
    if (isEditing.value) {
      ...
      const createPayload = {
        ...
        shipping_company: localInvoice.shipping_company,
        shipping_cost: calculateShippingCost(), // Send the calculated shipping cost
      };
